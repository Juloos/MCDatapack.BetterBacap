...
$execute if score max_len bac_display.vars matches 3 run \
    data modify storage bac_display:data page.$(team)[0].number_$(index) set value '[{"text":"$(progress)",$(progress_style)},{"text":"/",$(separator_style)},{"text":" ","bold":true},{"text":"ˈ","bold":false,"color":"#404040"},{"text":"$(max)",$(max_style)}]'
